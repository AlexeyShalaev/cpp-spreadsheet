...
std::vector<Position> Cell::GetParentCells() const {
    ...
    ans.reserve(parent_cells_.size());
    for (auto item: parent_cells_)
        ans.push_back(item);
    return ans;
}

void Cell::DeletePerents(const Position &cell) {
    parent_cells_.erase(cell);
}
        ...
        if (!color_.count(stack_.top().ToString())) {
            ...
            for (auto &item: sheet.GetCell(stack_.top())->GetReferencedCells()) {
                ...
                if (!color_.count(item.ToString()))
                    stack_.push(item);
                else if (!color_.at(item.ToString()))
                    return true;
            }

        } else {
            if (!color_.at(stack_.top().ToString()))
...
Cell::FormulaImpl::FormulaImpl(std::string_view str, Sheet &sheet, const Position &this_pos,
                               std::vector<Position> &child_cells_) : sheet_(
        sheet) /*:Impl(str.substr(1)) , value_(text_)*/  {
    str.remove_prefix(1);
